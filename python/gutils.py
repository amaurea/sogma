...
def find_last_crossing(hwp, ref, period=2*np.pi):
	"""Find the last sample where hwp moves past its initial value,
	ignoring period jumps. Raises an IndexError if there are no crossings"""
	ap    = device.anypy(hwp)
	dists = hwp-ref
	dsign = ap.diff(ap.sign(dists))
	jump  = ap.abs(ap.diff(hwp)) > period/2
	crossings= ap.where((dsign!=0)&~jump)[0]
	return crossings[-1]
...
def decimate_tod(tod, step, mul=32, contiguous=True):
	"""Decimate array tod along the last array, and also truncate it to make it
	a multiple of mul along that axis. Unless contiguous is false, the array
	will be made C-contiguous"""
	ap  = device.anypy(tod)
	tod = tod[...,::step]
	tod = tod[...,:tod.shape[-1]//mul*mul]
	if contiguous: tod = ap.ascontiguousarray(tod)
	return tod
...
class LinResamp:
	...
	def __init__(self, nin, nout):
		...
		self.x1 = x.astype(int)
		self.r2 = x-self.x1
		self.x2 = np.minimum(self.x1+1, nin-1)
		self.r1 = 1-self.r2
