...
# Want to support simple things like hor,equ,gal with minimal syntax.
# These define the up-direction and the center point.
# Also want to be able to define recentering operations. Maybe something
# like:
# "up=sys,from=pos,to=pos, where pos defaults to [0,0] and up= is
# implied if missing.
# pos specifies a point (not an orientation), and can be
# * sys:[ra,dec], where the sys part is optional, and defaults to the one used in up
# * objname
# This should be general enough to handle all my cases
def parse_sys(desc):
	info = {
		"up"  :{"sys":"equ", "pos":[0,np.pi/2]},
		"to"  :{"sys":"equ", "pos":[0,0]},
		"from":{"sys":"equ", "pos":[0,0]},
	}
	toks = desc.split(",")
	for i, tok in enumerate(toks):
		...
		key, val = subs
		if key not in ["up", "to", "from"]:
			raise ValueError("Only up, to and from can be used when building a coordinate system, but got '%s'" % str(key))
		info[key] = _parse_sys_pos(val, default_sys=info["up"]["sys"])
	return info

def _parse_sys_pos(pdesc, default_sys="equ"):
	import re
	toks = pdesc.split(":")
	if len(toks) == 1: toks = [default_sys]+toks
	if len(toks) != 2:
...
#def eval_sys(sysinfo, ctimes):
#	"""Given a coordinate system info from parse_sys, returns a 
